...
export const Texto3DInput = ({ isCollapsed }) => {
  ...
  const handleCreatePrediction = async () => {
    if (!generationName.trim() || !userPrompt.trim()) return;
    setIsResultReady(false);
    const payload = {
      generationName,
      prompt: userPrompt,
  ...
  const handleRegenerateClick = () => {
    if (isResultReady && prediction_text3d_result) {
  ...
  const handleConfirmRegenerate = async () => {
    ...
    if (!user || !prediction_text3d_result) return;
  ...
  const isButtonDisabled =
    isJobActive || !generationName.trim() || !userPrompt.trim();
  ...
  return (
    <>
      ...
      <RegenerateConfirmationModal
        ...
        generationName={prediction_text3d_result?.generation_name || ""}
      />
    </>
  );
};
