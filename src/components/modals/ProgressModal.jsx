...
export const ProgressModal = ({ show, jobStatus }) => {
  ...
  const getMessage = () => {
    switch (jobStatus?.status) {
      ...
      case "processing":
        return jobStatus.step || t("progress_modal.processing_message_default");
      default:
      ...
      <Modal.Body className="p-0 bg-transparent">
        <div className="text-center bg-white dark:bg-principal rounded-2xl py-8 px-6 border-2 border-gray-200 dark:border-linea/50 shadow-2xl shadow-azul-gradient/10">
          <div className="flex flex-col items-center">
            ...
            <p
              key={getMessage()}
              className="text-sm text-gray-500 dark:text-gray-400 animate-[fadeIn_0.5s_ease-in-out_0.2s]"
            >
              {getMessage()}
            </p>
          </div>
