...
const Modal = ({ isOpen, onClose, children }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-50 bg-principal rounded-lg max-w-3xl mx-4">
        {children}
      </div>
    </div>
  );
...
function Model({ url, showWireframe, showTexture, onTextureLoad }) {
...
export const Imagen3DResult = ({ prediction_img3d_result }) => {
  ...
  useEffect(() => {
    if (prediction_img3d_result?.make3d) {
  ...
  return (
    <div className="w-full h-full bg-principal rounded-r-lg relative">
      {/* Overlay de espera */}
      ...
      <div className="relative h-full">
        ...
        <Canvas camera={{ position: [0, 0, 1.7] }} className="h-full">
          <Suspense fallback={null}>
            ...
            {prediction_img3d_result?.make3d && (
              <Model
                url={prediction_img3d_result.make3d[1]}
